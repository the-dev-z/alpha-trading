...
Alpha Trading is an AI-powered cryptocurrency trading bot that uses large language models (LLMs) to make trading decisions on futures markets.
...
Yes! Alpha Trading supports running multiple traders with different configurations, AI models, and trading strategies.
...
No! Alpha Trading has a web UI for all configuration. However, basic command line knowledge helps with setup and troubleshooting.
...
**Recommended**: Yes, use a subaccount dedicated to Alpha Trading for better risk isolation. However, note that some subaccounts have restrictions (e.g., 5x max leverage on Binance).
...
**Why**: Alpha Trading uses `PositionSide(LONG/SHORT)` which requires Hedge Mode.

See [Issue #202](https://github.com/the-dev-z/alpha-trading/issues/202) and [Troubleshooting Guide](TROUBLESHOOTING.md#-only-opening-short-positions-issue-202).
...
Yes, to some extent. Alpha Trading provides historical performance feedback in each decision prompt, allowing the AI to adjust its strategy.
...
2. Search [existing issues](https://github.com/the-dev-z/alpha-trading/issues)
...
### Can I contribute to Alpha Trading?
...
Open a [Feature Request](https://github.com/the-dev-z/alpha-trading/issues/new/choose) with your idea!
